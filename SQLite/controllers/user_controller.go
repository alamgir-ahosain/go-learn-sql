...
import (
	...
	"github.com/alamgir-ahosain/learn-sql/SQLite/models"
)
...
func CreateUser(sid, name string, cgpa float32) {
	...
	row, err := dbsqlite.DB.Exec(query, sid, name, cgpa)
	CheckError(err)
...
func ReadUserById(id int) (*models.User, error) {
	...
	query := dbsqlite.DB.QueryRow("select * from users where id=?", id)
...
func ReadUserByField(sid string) ([]models.User, error) {
	...
	query, err := dbsqlite.DB.Query("select * from users where sid=?", sid)
	if err != nil {
		return nil, err
	}
	defer query.Close()
...
func UpdateUser(id int, NewName string) {
	...
	row, err := dbsqlite.DB.Exec(query, NewName, id)
	CheckError(err)
...
func DeleteUser(id int) {
	...
	row, err := dbsqlite.DB.Exec(query, id)
	CheckError(err)
