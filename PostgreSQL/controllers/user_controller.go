...
import (
	...
	"github.com/alamgir-ahosain/learn-sql/PostgreSQL/models"
)
...
func CreateUser(sid, name string, cgpa float32) {
	...
	query := "insert into users(sid,name,cgpa) values($1,$2,$3) returning id"
	var id int
	err := db.DB.QueryRow(query, sid, name, cgpa).Scan(&id)
	CheckError(err)
...
func ReadUserById(id int) (*models.User, error) {
	...
	query := db.DB.QueryRow("select * from users where id=$1", id)
...
func ReadUserByField(sid string) ([]models.User, error) {
	...
	query, err := db.DB.Query("select * from users where sid=$1", sid)
	if err != nil {
		return nil, err
	}
	defer query.Close()
